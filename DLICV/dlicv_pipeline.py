...
def run_dlicv_pipeline(
    ...
    if not os.path.exists(model_folder):
        # HF download model
        logging.info("DLICV model not found, downloading...")

        from huggingface_hub import snapshot_download

        local_src = Path(__file__).parent
        snapshot_download(repo_id="nichart/DLICV", local_dir=local_src)
        logging.info("DLICV model has been successfully downloaded!")
    else:
    ...
    logging.info("DLICV Process Done!")
